...
def init_state():
    ...
    if st.session_state.get("app_state_version") != 6:
        st.session_state["app_state_version"] = 6
...
def _normalise_values(values):
    """Keep profile values JSON-safe for Supabase JSONB/Auth metadata."""
    if values is None:
        return []
...
def load_user_data(user_id, email, auth_metadata=None):
    ...
    metadata_values = _normalise_values(metadata.get("values"))
...
def save_profile():
    ...
    profile_data = {
        ...
        "city": st.session_state.get("city", ""),
        "audience": st.session_state.get("audience", ""),
        "values": _normalise_values(st.session_state.get("values")),
    }
