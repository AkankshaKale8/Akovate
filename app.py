...
from utils.ui import inject_css, logo
...
def render_sidebar():
    with st.sidebar:
        ...
        if st.session_state.get("authenticated"):
            ...
            role_index = (
                ROLES.index(current_role) if current_role in ROLES else 0
            )
...
def render_onboarding():
    ...
    st.caption(
        "Your fields adapt automatically to the selected workspace role."
    )
    ...
    with st.form("role_onboarding"):
        if role == "Brand":
            ...
            city = st.text_input(
                "Primary market",
                value=st.session_state.get("city", "Hyderabad"),
            )
            ...
            values = st.multiselect(
                ...
                default=st.session_state.get(
                    "values",
                    ["Growth", "Creator authenticity", "Sustainability"],
                ),
            )
            ...
            audience = st.text_area(
                "Audience profile",
                value="Gen Z and young millennials",
            )
            values = st.multiselect(
                "Creator strengths",
                [
            ...
            audience = st.text_area(
                ...
                value="D2C and digital-first brands",
            )
            values = st.multiselect(
                "Capabilities",
                [
            ...
            audience = st.text_area(
                "Partner ecosystem",
                value="Brands, agencies and campaign teams",
            )
            values = st.multiselect(
                "Capabilities",
                [
            ...
            audience = st.text_area(
                "Operational scope",
                value="Akovate platform ecosystem",
            )
            values = st.multiselect(
                "Admin permissions",
                [
